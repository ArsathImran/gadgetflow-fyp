                                            ...
                                            <td class="px-6 py-4 text-sm">
                                                ...
                                                @if ($rental->status === 'pending')
                                                    <form method="POST" action="{{ route('customer.rentals.cancel', $rental) }}" class="mt-3" onsubmit="return confirm('Are you sure you want to cancel this rental request?');">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit" class="inline-flex items-center rounded-md border border-indigo-200 px-3 py-2 text-xs font-body font-semibold text-indigo-700 transition hover:bg-indigo-50">
                                                            Cancel Request
                                                        </button>
                                                    </form>
                                                @endif
