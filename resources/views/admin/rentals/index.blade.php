                        ...
                        <div class="md:hidden space-y-3">
                            @foreach ($rentals as $rental)
                                @php
                                    ...
                                    $itemThumbnail = $rental->isBundle() ? $rental->bundle?->image : $rental->gadget?->image;
                                @endphp
                                <div id="rental-{{ $rental->id }}-mobile" class="rounded-2xl border border-slate-200 bg-white p-4 {{ ($rental->status === 'pending' || $rental->payment_status === 'pending') ? 'bg-amber-50/50' : '' }}">
                                    <div class="flex items-center gap-3">
                                        <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-indigo font-display text-sm font-semibold text-white">
                                            {{ strtoupper(substr($rental->user?->name ?? '?', 0, 1)) }}
                                        </span>
                                    ...
                                    <div class="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-4">
                                        @if ($rental->status === 'pending')
                                            <form method="POST" action="{{ route('admin.rentals.approve', $rental) }}">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="inline-flex items-center gap-1.5 rounded-md border border-green-300 px-3 py-2 text-sm font-body text-green-700 transition hover:bg-green-50">
                                                    <x-icon-check class="h-3.5 w-3.5" />
                                                    Approve
                                                </button>
                                            </form>
                                            ...
                                            <form method="POST" action="{{ route('admin.rentals.reject', $rental) }}">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="inline-flex items-center gap-1.5 rounded-md border border-red-300 px-3 py-2 text-sm font-body text-red-700 transition hover:bg-red-50">
                                                    <x-icon-x class="h-3.5 w-3.5" />
                                                    Reject
                                                </button>
                                            </form>
                                        ...
                                        @if ($rental->pickup_type === 'delivery' && $rental->payment_status === 'pending')
                                            @if (count($paymentProofs))
                                                <button
                                                    ...
                                                    x-data
                                                    x-on:click="$dispatch('open-modal', 'verify-payment-{{ $rental->id }}')"
                                                    class="inline-flex items-center gap-1.5 rounded-md border border-indigo-300 px-3 py-2 text-sm font-body text-indigo-700 transition hover:bg-indigo-50"
                                                >
                                                    <x-icon-wallet class="h-3.5 w-3.5" />
                                                    Verify Payment
                                                </button>
                                        ...
                                        @if ($rental->status === 'approved')
                                            <a href="{{ route('admin.scan') }}" class="inline-flex items-center gap-1.5 rounded-md border border-cyan-300 px-3 py-2 text-sm font-body text-cyan-700 transition hover:bg-cyan-50">
                                                <x-icon-qr class="h-3.5 w-3.5" />
                                                Scan QR
                                            </a>
                                        @endif

                                        <a href="{{ route('admin.rentals.show', $rental) }}" class="inline-flex items-center gap-1.5 rounded-md border border-indigo-200 bg-white px-3 py-2 text-sm font-body text-indigo-700 transition hover:bg-indigo-50">
                                            <x-icon-eye class="h-3.5 w-3.5" />
                                            View
                                        </a>
                                    </div>
